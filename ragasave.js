...
(function () {
    ...
    const RagasaveColors = {
        primary: {bg: 'rgba(1, 108, 244,1)', active: 'rgb(0, 92, 208)', text: '#fff', sh: 'rgba(1, 108, 244,0.34)'},
        danger: {bg: 'rgba(255, 17, 0,1)', active: 'rgb(199, 13, 0)', text: '#fff', sh: 'rgba(255, 90, 38,0.34)'},
        warn: {bg: 'rgba(255, 90, 38, 1)', active: 'rgb(232, 83, 36)', text: '#fff', sh: 'rgba(255, 90, 38, 0.34)'},
        success: {bg: 'rgba(2, 154, 71, 1)', active: 'rgb(1, 128, 58)', text: '#fff', sh: 'rgba(2, 154, 71, 0.34)'},
        info: {bg: 'rgba(255, 190, 0,1)', active: 'rgb(228, 170, 0)', text: '#fff', sh: 'rgba(255, 190, 0,0.34)'},
        blue: {bg: 'rgba(1, 108, 244,1)', active: 'rgb(0, 100, 226)', text: '#fff', sh: 'rgba(1, 108, 244,0.34)'},
        red: {bg: 'rgba(255, 17, 0,1)', active: 'rgb(199, 13, 0)', text: '#fff', sh: 'rgba(255, 90, 38,0.34)'},
        green: {bg: 'rgba(2, 154, 71, 1)', active: 'rgb(1, 128, 58)', text: '#fff', sh: 'rgba(2, 154, 71, 0.34)'},
        orange: {bg: 'rgba(255, 90, 38, 1)', active: 'rgb(232, 83, 36)', text: '#fff', sh: 'rgba(255, 90, 38, 0.34)'},
        yellow: {bg: 'rgba(255, 190, 0,1)', active: 'rgb(228, 170, 0)', text: '#fff', sh: 'rgba(255, 190, 0,0.34)'},
        white: {bg: 'rgba(255, 255, 255,1)', active: '#fff', text: '#9E9E9E', sh: 'rgba(124, 124, 124,0.34)'},
        lightgrey: {bg: '#dcdcdc', active: '#c3c3c3', text: '#757575', sh: 'rgba(220, 220, 220, 0.34)'},
        lightblue: {bg: 'rgb(100, 149, 237)', active: '#5b88d8', text: '#fff', sh: 'rgba(100, 149, 237,0.34)'},
        default: {bg: 'rgba(144, 164, 174, 1 )', active: 'rgb(117, 132, 140)', text: '#f1f1f1', sh: 'rgba(144, 164, 174, 0.34 )'},
        grey: {bg: 'rgb(105, 105, 105)', active: 'rgb(93, 93, 93)', text: '#fff', sh: 'rgba(105, 105, 105,0.34)'},
        darkblue: {bg: 'rgb(72, 61, 139)', active: 'rgb(58, 49, 111)', text: '#fff', sh: 'rgba(72, 61, 139,0.4)'},
        crimson: {bg: 'rgb(237, 20, 61)', active: 'rgb(197, 19, 53)', text: '#fff', sh: 'rgb(237, 20, 61,0.34)'},
        dark: {bg: '#000', active: 'rgb(30, 30, 30)', text: '#f8f8f8', sh: 'rgba(0, 0, 0, 0.34)'},
        pink: {bg: 'rgb(255, 20, 147)', active: 'rgb(202, 18, 117)', text: "#fff", sh: 'rgba(255, 20, 147,0.34)'},
        darkgreen: {bg: 'rgba(95, 125, 139,1)', active: 'rgb(75, 100, 111)', text: '#f8f8f8', sh: 'rgba(96, 125, 139, 0.34)'}
    };
    const R = {};
    const _u = "undefined";
    const _f = "function";
    ...
    function childOf(selector,child) {
        ...
        if(!elem || !elem.nodeType || !selector){return false}
    ...
    (function () {
        let AjaxEvent = {"ajaxStart": "loadstart", "ajaxEnd": "loadend", "ajaxProgress": "progress", "ajaxError": "error"};
        ...
        ;
        let AjaxChangeEvent = {"ajaxSuccess": 4, "ajaxBefore": 0, "ajaxProcess": 3};
        ...
        position: function (c) {
            if (c && typeof c === _f) {
                _rc.evaluate(this, function (e, i) {
                    ...
                    x = {width: x.width, height: x.height, top: x.top, left: x.left, bottom: x.bottom, right: x.right, x: x.x, y: x.y};
                    c(e, x);
                });
                return this;
            } else {
                if (this[0] && this[0].nodeType === 1) {
                    ...
                    let p = {width: ep.width, height: ep.height, top: ep.top, left: ep.left, bottom: ep.bottom, right: ep.right, x: ep.x, y: ep.y};
                    return p;
                }
            }
        },
    ...
    _rc.each({offsetX: "offsetLeft", offsetY: "offsetTop", offsetWidth: "offsetWidth", offsetHeight: "offsetHeight"}, function (p, v) {
        Ragasave.extend({
            [p]: function (c) {
    ...
    Ragasave.style = function (c) {
        ...
        if (!doc.querySelector('style[rcStyle]')) {
            var sn = createNode({tag: "style", attr: {type: "text/css", auther: "Ragasave", rcStyle: ''}, text: style});
    ...
    dragAndDrop: function (o) {
        ...
        function draggable() {
            ...
            fn.dragStart(function (element) {
                ...
                fn.setData(fn.dataKey, JSON.stringify({dataId : fn.dataId,mode : fn.mode}));
    ...
    return createNode({
        ...
        html : [createNode({
            ...
            html : (function(){
                ...
                if(active === false){
                    Ragasave(tabs[0]).find('.RagasaveUI-container-tab-btn-cw').attr({activeTab : ''});
                }
                return tabs;
            }())
        }),createNode({
    ...
    assets._Node = createNode({
        ...
        html : [createNode({
            ...
            bind : [{
                type : 'mouseover',
                handler : function(){
                    if(!o.moveable&&o.moveable !== true){Ragasave(this).off('mouseover','Ragasave-UI-bind-event-self-xsfsef');return}
                    var that = this;
...
(function () {
    ...
    Ragasave.UI.Controls = {
                                        ...
                                        html: (function () {
                                            ...
                                            if (o.option && objType(o.option) === 'array') {
                                                Ragasave(o.option).each(function (e, i) {
                                                    if(e.default&&e.default === true){defaultValue = i;}
                                                    _NodeList.push(createNode({
                                                        ...
                                                        bind : [{
                                                            ...
                                                            handler : function(){
                                                                ...
                                                                var l = sb.find('[sb-label]');
                                                                var ib = sb.find('input');
                                                                if(ib.val() === sl.attr('value')){return false}
                                            ...
                                            },
                                            {
                                                ...
                                                handler: function () {
                                                    ...
                                                    if (this.hasAttribute('min') && Number(this.value.length) >= Number(this.getAttribute('min'))) {
                                                        Ragasave(this).parent(3).find('[TextFieldAnimBox]').css({backgroundColor: 'transparent'});
                                                        Ragasave(this).parent(3).find('[TextFieldAnimate]').css({visibility: 'visible'});
        ...
        throwError : function(s,m){
            ...
            Elems.each(function(e){
                ...
                if(!Ragasave(e).parent(3).hasAttr('RagasaveControlUI')){return}
                Ragasave(e).parent(3).attr({
                    showError : true
                });
                if(Ragasave(e).parent(3).hasAttr('TextField')){
                    Ragasave(e).parent(3).find('[TextFieldAnimBox]').css({backgroundColor: 'red'});
                    Ragasave(e).parent(3).find('[TextFieldAnimate]').css({visibility: 'hidden'});
    ...
    function validate(form, z) {
        ...
        function hideError(e){
            Ragasave(e).parent(3).find('[TextFieldAnimBox]').css({backgroundColor: 'transparent'});
            Ragasave(e).parent(3).find('[TextFieldAnimate]').css({visibility: 'visible'});
            Ragasave(e).parent(3).find('[TextFieldLabel] label').css({
                color: 'inherit'
            });
